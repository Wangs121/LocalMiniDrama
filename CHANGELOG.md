...
- **对话式 AI 对象修改**：角色、场景、道具和分镜均可在保存后通过连续对话调整；支持人工补建遗漏对象后继续让 AI 完善，并保留会话历史、候选版本和详细 AI 回复。
- **本地字段差异审阅**：AI 只返回完整候选对象，前后差异由本地确定性计算；创作者可逐字段勾选应用，媒体相关字段变化会提示已有图片或视频可能过期。
- **分镜 AI 修改面板**：列表与画布共用完整分镜编辑对话框，覆盖内容、关联、镜头、画面和提示词字段。
- **内容类型创作**：新建项目可选择短剧或主题视频，并根据内容类型提供对应的脚本结构、创作目的和生成约束。

### 优化

- **分镜与导出分辨率分离**：Seedance 分镜视频的 `480p / 720p / 1080p` 选择移至「5. 分镜生成」配置行；下载后的分镜视频保留服务商返回的原始像素，不再强制放大为固定尺寸。
- **整集导出分辨率**：在「合成视频」区域增加独立导出设置，支持 360p、480p、540p、720p、1080p、1440p、2160p、4K 和自定义最长边（最大 3840 像素）；合成阶段由 ffmpeg 统一缩放并补边，字幕、配音和水印继续在成片上处理。
- **分镜时长动态规划**：AI 根据对白长度、动作复杂度、情绪停顿和叙事作用，在 4–15 秒内独立决定每条新分镜时长；项目秒数仅用于估算镜数，不再成为单镜目标或下限。
- **AI 修改结果聚焦**：首次回复和历史记录优先展示字段前后对比，详细 AI 回复按需展开，长内容可完整滚动查看。
- **新建项目移动端适配**：限制对话框高度并改善小屏滚动，避免内容超出视口。

### 修复

- **分镜 AI 枚举合同**：生成和自动修复提示词共享后端字段合同，明确 `angle_h`、`angle_v`、`angle_s` 等合法枚举值，修复“无效的 angle_h”导致 AI 修改服务不可用的问题。
- **AI 修改可靠性**：加强关系 ID、阶段范围、候选字段和会话状态校验；处理快照过期、请求重试、并发请求、清空历史与失败审计。
- **分镜时长汇总一致性**：总时长改为按归一化并实际保存的分镜行汇总，避免越界或无效 AI 时长造成剧集时长不一致。

### 文档

- 根目录 `README.md`、`docs/en.md`、`index.html`、各子包 README 同步 **v1.2.9**。
- `frontweb` / `backend-node` / `desktop` 的 `package.json` 与 lock 文件顶层 **version** 统一为 **1.2.9**。
...
- **通用创作 Skill 系统**：支持 ZIP/浏览器目录安全导入、manifest 校验、预览、启停、用户包删除和项目级选择；按单次请求、项目、全局优先级路由，并记录版本、章节、截断和内容哈希
- **六个内置影视能力包**：将 MapleShaw 的 MIT 内容人工精炼为电影镜头、视觉美学、创意策略、剪辑节奏、分镜规划和长视频规划；厂商参数继续留在适配层
- **内置 Skill 2.0 深度扩展**：恢复并通用化 MapleShaw 原仓库的相机四维编码、美学约束、创意模式、剪辑公式、分镜板方法、参考图运动设计和长视频流水线；阶段上下文预算提升为单引用 64K、总计 128K 字符
...
- **Seedance 2.0 官方提示词编排**：全能模式改用 `镜头1 / 镜头2` 顺序镜头格式，整段时长仅作为复杂度边界，不再强制内部镜头精确分秒；每镜限制一个主要运镜，并补充主体稳定特征、动作物理因果、情绪外化、素材职责与无字幕/Logo/水印约束
- **任务服务**：新增 `taskService` 与 `/api/task` 路由，统一异步任务状态查询

### 修复

- **分镜图片数量上限**：修复分镜图片生成时数量限制未正确生效的 bug
- **Seedance 2.0 首尾帧约束**：修复全能模式存在多参考图时丢弃 `first_frame_url` / `last_frame_url` 的问题；方舟请求现在分别使用 `first_frame`、`last_frame` 和 `reference_image`，并对最多 9 张输入图片去重
- **全能分镜多行结构**：修复批量分镜生成后保存 `universal_segment_text` 时将换行压成空格的问题；旧时间戳草稿仍可读取，并可在重新生成或润色时转换为新格式
