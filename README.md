...
| 步骤 | 功能 | 说明 |
|:----:|------|------|
| 1 | **脚本生成** | 选择短剧或主题视频，按目标时长和自定义风格生成脚本 |
...
- **短剧**：支持多集生成；每集按目标时长组织对白、动作和场景。结尾是否收束、留余味、反转或承接下一集，由项目“剧集风格”和输入资料决定，不默认强制悬念。
- **主题视频**：用于宣传、讲解和科普等单集内容，继续复用“脚本 → 分镜 → 首尾帧 → 视频 → 合成”流程。主题视频项目固定为一集。
- **项目设置**：可设置 10–600 秒目标时长和项目级自定义风格。自定义风格会注入脚本生成提示词，但不能覆盖事实、安全要求或 JSON 输出格式。
- **兼容旧项目**：未保存内容类型的已有项目按短剧处理；项目已有分集后不能切换主内容类型。
