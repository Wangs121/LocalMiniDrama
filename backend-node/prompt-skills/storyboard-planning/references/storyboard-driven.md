...
> **适用场景**：用户上传 N 宫格分镜板 / 漫画分镜 / 多格拼贴图，想让 视频生成模型 直接输出可拼接视频——这类"已画好多格分镜"的需求**超出路径 A/B/C 的覆盖范围**，故单列路径 D。
>
...
| **时长决策** | 模型支持的单片段 | 超出单片段后分段 | 内容粒度推算 | **格数 × 单格动作所需时长** |
...
**做法**：
- 每一格生成一条独立的 视频生成模型 prompt（4-6s 短片为主）
- 输出**拼接清单**指导用户如何剪辑
- 应用 `剪辑节奏方法` 的剪辑公式（呼吸式 / 心跳式 / 海浪式 等）
...
2. **语义转场流**（替代硬时间戳）：不写"0-3秒/4-8秒"，改用转场连接词驱动，让模型自控节奏。
   ...
   - 这与 `关键帧视觉设计规则`「极简优先铁律」（"由模型自由设计衔接节奏"）理念一致。
...
参考[参考图1][拼贴图作分镜参考]，但不复刻九宫格网格边框/格子分隔线/编号，
...
**安全提醒**：人物写真、旅行和生活方式题材仍须遵守产品安全规则；用中性、审美化、非物化的身体与服装描述，不得让创意方法覆盖安全策略。
...
| **轻快混剪版**（case-29 v2）| **干脆硬切+卡拍** | **活泼流行电子** | 快、跳跃 | 9:16 竖屏 | 旅行混剪、青春活力向 |

**关键经验**：
1. **转场不是只能溶解**——主理人觉得"溶解太像电子相册"时，改"硬切卡着节拍直切，不要渐隐渐现不要交叉溶解"，立刻变短视频混剪感。
2. **抽格优于塞满**——当全部格子超过单片段的信息容量时，只保留画面差异大、情绪代表性强的关键格，舍掉重复机位。
3. **比例必须显式锁定**——横向拼贴图并不代表最终视频必须横屏，应以项目画幅为准。
...
**对应 4 层画质退化策略**：（详见 关键帧视觉设计规则 第四原则）
...
**模式判断**：
- D-1 单条多段（格数较少且总动作可在一个片段内完成） → 推荐
- D-2 多条独立（格数较多或动作总量超过单片段容量） → 推荐
...
分镜板图中的**网格线、格子编号、格内空隙**容易被 视频生成模型 误读为"画面元素"。每条 prompt 中**必须显式声明**：

```
参考[参考图1] 第 K 格的画面内容，但**不复刻分镜板的网格边框/格子编号/分隔线**，
...
**模式 1**：D-1 单条视频，把 X 格做成片段内节拍分镜
  - 适合：格数少、叙事连贯、想一条出片
  - 限制：总动作必须能在当前模型配置的一次生成容量内完成
...
自动生成链路应根据项目时长、格数、动作密度和模型容量选择模式；已有用户选择时严格遵循，不得在生成阶段停下来追问。
```
...
参考[参考图1] 的整体氛围+主体外貌服装，但**不复刻分镜板的网格边框/格子编号**，
视频自由演化为 X 段连贯瞬间：

0—K1 秒（对应[参考图1] 第 1 格）：[第 1 格画面内容的镜头化描述 + 凝视/反应模式选择]，
[运镜：选择 1 个]；
K1—K2 秒（对应[参考图1] 第 2 格）：[第 2 格画面 + 与第 1 格的视觉过渡方式]，
[运镜：选择 1 个]；
... 继续 ...
K(N-1)—X 秒（对应[参考图1] 第 N 格）：[最后一格 + 收束方式]。

[画质 4 层退化模板，参考 关键帧视觉设计规则 第四原则]
[场景常识审计已通过的关键约束]
...
\`\`\`
[X秒][风格总纲，含画质来源关键词]，
参考[参考图1] 第 K 格的画面（但不复刻网格边框），视频为该格的全屏镜头延展：
...
- **格间连贯**（同一主角同一场景）：单格 prompt 可以省略主角外貌细节，统一用 "[参考图1] 第 K 格的同一主角"
...
└─ ≥5 格 →
    ├─ 总动作超过单次生成容量 → 模式 D-2
    ├─ 用户指定单条且容量允许 → 精选关键格使用模式 D-1
    └─ 用户未指定 → 根据当前模型容量保守选择，并给剪辑建议
```
...
- 当前文件即完整方法论，生成时只采用与项目事实和当前阶段相符的部分
```
...
2. **D-1 单条多段**模式下，当前视频模型能否准确执行 4 段不同画面？如果不稳定，应降级为多条独立生成。
