...
function generateStoryboard(db, log, episodeId, options) {
  ...
  const { model, style, storyboard_count, video_duration, aspect_ratio, include_narration, universal_omni_storyboard, prompt_skill_ids } = options || {};
  // 转换可能为字符串的数字
  ...
  return episodeStoryboardService.generateStoryboard(
    ...
    Array.isArray(prompt_skill_ids) ? prompt_skill_ids : undefined
  );
}
